...
def _run_npp(process_count, arrival_time, burst_time, priority_list):
    ...
    current_time=0; done=0; gantt_chart=[]; gantt_time=[0]
    while done < process_count:
        ...
        label=f"P{idx+1}"
        if gantt_chart and gantt_chart[-1][0]==label: gantt_chart[-1][2]=current_time
        else: gantt_chart.append([label,start_exec,current_time])
        gantt_time.append(current_time); completed[idx]=True; done+=1
    total_turnaround=0; total_waiting=0; turnaround_time=[]; waiting_time=[]
...
def priority_gui():
    ...
    try:
        ...
        win.attributes('-zoomed', True)  # Maximizes on Linux
...
if __name__ == "__main__":
    root=tk.Tk(); root.withdraw(); priority_gui(); root.mainloop()